...
class PicardSeidel:
    ...
    def pseidel(self, fs,x,c, epsilon = 1.0e-5):
        ini_x = copy.deepcopy(x)
        self.txt += '\n\n'+self.sep+'\t\t\tX\t\t\t\tError Relativo\n'+self.sep+'\n'
        jact = 1.0
        for i in range(30):
            n = len(x)
            ...
            xn1 = copy.deepcopy(x)
            #itera en cada dimension
            for j in range(n):
                #las dimensiones anteriores, usa el ultimo valor calculado
                xtemp = []
                for k in range(j):
                    xtemp.append(x[k])
                for k in range(j,n):
                    xtemp.append(xn1[k])
                x[j] = x[j] + c[j]*self.f(xtemp,fs[j])
                xn1 = copy.deepcopy(x)
                print x
